...
export function getModelLimits(modelId: string, provider?: string): ModelLimits | undefined {
  ...
  if (provider) {
    ...
    const exactMatch = MODEL_LIMITS.find(
      limit =>
        limit.provider.toLowerCase() === normalizedProvider &&
  ...
  const modelMatch = MODEL_LIMITS.find(limit => limit.modelId.toLowerCase() === normalizedModelId);
  if (modelMatch) {
    return modelMatch;
  }
...
function findFuzzyMatch(modelId: string, provider?: string): ModelLimits | undefined {
  ...
  if (baseModelId !== modelId) {
    ...
    if (provider) {
      const providerMatch = MODEL_LIMITS.find(
        limit =>
          limit.provider.toLowerCase() === provider && limit.modelId.toLowerCase() === baseModelId
      );
      if (providerMatch) {
    ...
    // 跨 provider 匹配
    const baseMatch = MODEL_LIMITS.find(limit => limit.modelId.toLowerCase() === baseModelId);
    if (baseMatch) {
      return baseMatch;
    }
  }
  ...
  const candidates = MODEL_LIMITS.filter(limit => {
  ...
  if (candidates.length > 0) {
    ...
    if (provider) {
      const providerMatch = candidates.find(limit => limit.provider.toLowerCase() === provider);
      if (providerMatch) {
        return providerMatch;
      }
    }

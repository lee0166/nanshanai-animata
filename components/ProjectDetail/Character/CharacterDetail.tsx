...
const CharacterDetail: React.FC<CharacterDetailProps> = ({ asset, onUpdate, projectId }) => {
  ...
  useEffect(() => {
    ...
    if (needsUpdate) {
      console.log(
        `[CharacterDetail] Auto-correcting params for model ${modelId}: Res ${resolution}->${newRes}, Ratio ${aspectRatio}->${newRatio}`
      );
    ...
    const checkActiveJob = async () => {
      ...
      try {
        ...
        if (activeJobs.length > 0) {
          console.log(
            `[CharacterDetail] Found ${activeJobs.length} active jobs for asset ${asset.id}`
          );
          setGenerating(true);
        ...
        if (job.status === JobStatus.COMPLETED || job.status === JobStatus.FAILED) {
          ...
          if (job.status === JobStatus.COMPLETED) {
            ...
            try {
              ...
              console.log(
                `[CharacterDetail] Reloaded asset ${asset.id}. Generated images count: ${updatedAsset?.generatedImages?.length || 0}`
              );
  ...
  const handleGenerate = async () => {
    ...
    const ageLabel =
      t.character.ageOptions[ageGroup as keyof typeof t.character.ageOptions] || ageGroup;
    ...
    try {
      console.log(
        `[CharacterDetail] Submitting generation job for asset: ${asset.id} with count: ${generateCount}`
      );

...
import {
  ...
  DefaultStrategy,
  VolcImageRequest,
} from '../adapters/volcengine/strategies';
...
export class VolcengineProvider extends BaseProvider {
  ...
  async generateImage(
    ...
    try {
      ...
      if (referenceImages.length > 0) {
        console.log(
          '[VolcengineProvider] first image preview:',
          referenceImages[0].substring(0, 50)
        );
      }
      ...
      // 3. Prepare Request
      const body = strategy.prepareRequest(
        prompt,
        config,
        loadedImages,
        size,
      ...
      console.log(
        '[VolcengineProvider] body.image field:',
        body.image
          ? `present (length: ${Array.isArray(body.image) ? body.image.length : 1})`
          : 'not present'
      );
      if (body.image) {
      ...
      if (data.data && data.data.length > 0) {
        ...
        if (validResults.length > 0) {
          ...
          const sanitizedData = originalData?.map((item: any) => ({
            ...item,
  ...
  async generateVideo(
    ...
    try {
      ...
      const useRoles =
        config.capabilities?.supportsEndFrame === true ||
        config.capabilities?.supportsStartFrame === true;
      // Check for multi-reference image support (role: reference_image)
      const useMultiRef =
        config.capabilities?.supportsReferenceImage === true && config.type === 'video';
      ...
      if (
        useMultiRef &&
        extraParams?.referenceImages &&
        Array.isArray(extraParams.referenceImages)
      ) {
      ...
      if (extraParams) {
        ...
        // Return Last Frame
        if (extraParams.returnLastFrame !== undefined)
          body.return_last_frame = extraParams.returnLastFrame;
      ...
      const shouldGenerateAudio =
        extraParams?.generateAudio !== undefined
          ? extraParams.generateAudio
    ...
    for (let i = 0; i < maxAttempts; i++) {
      try {
        ...
        if (['failed', 'cancelled', 'expired'].includes(data.status)) {
          return {
            success: false,
            error: `Video generation ${data.status}: ${data.error?.message}`,
            metadata: data,
          };
        }
      } catch (e) {

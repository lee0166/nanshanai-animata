 ...
 * 评估提示词的细节丰富度
 */
function evaluateDetailLevel(
  prompt: string,
  assetType: AssetType
): { level: '不足' | '适中' | '丰富'; count: number } {
  ...
  if (assetType === 'role') {
    ...
    detailKeywords = [...materials.wood, ...materials.metal, ...lighting];
  } else {
    // item
    detailKeywords = [...materials.metal, ...materials.wood];
  }
 ...
 * 评估提示词的具体性
 */
function evaluateSpecificity(prompt: string): {
  level: '笼统' | '具体' | '非常具体';
  hasVague: boolean;
} {
...
function evaluateCoherence(prompt: string): { level: '割裂' | '连贯' | '非常连贯'; score: number } {
  ...
  // 检查是否有机械拼接痕迹
  const hasIncoherent = parts.some(
    part => part.length < 3 || (part.includes('脚穿') && parts.indexOf(part) < parts.length - 2)
  );
...
export function evaluateItemPrompt(item: ScriptItem): PromptQualityFeedback {
  const prompt = item.visualPrompt || item.description || '';

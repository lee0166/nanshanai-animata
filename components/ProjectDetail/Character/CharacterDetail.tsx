...
import { CharacterAsset, GeneratedImage, AssetType, Job, JobStatus } from '../../../types';
...
import { getRoleImagePrompt, getDefaultStylePrompt, DefaultStylePrompt } from '../../../services/prompt';
...
const CharacterDetail: React.FC<CharacterDetailProps> = ({ asset, onUpdate, projectId }) => {
    ...
    const staticModel = DEFAULT_MODELS.find(m => m.id === runtimeModel?.id || m.modelId === runtimeModel?.modelId);
    ...
    useEffect(() => {
        ...
        if (needsUpdate) {
            console.log(`[CharacterDetail] Auto-correcting params for model ${modelId}: Res ${resolution}->${newRes}, Ratio ${aspectRatio}->${newRatio}`);
    ...
    useEffect(() => {
        setGenerating(false);
        setName(asset.name);
        setGender(asset.gender || 'unlimited');
        setAgeGroup(asset.ageGroup || 'unknown');
        ...
        const checkActiveJob = async () => {
            ...
            try {
                ...
                if (activeJobs.length > 0) {
                    console.log(`[CharacterDetail] Found ${activeJobs.length} active jobs for asset ${asset.id}`);
                    setGenerating(true);
                ...
                if (job.status === JobStatus.COMPLETED || job.status === JobStatus.FAILED) {
                    ...
                    if (job.status === JobStatus.COMPLETED) {
                        ...
                        try {
                            ...
                            console.log(`[CharacterDetail] Reloaded asset ${asset.id}. Generated images count: ${updatedAsset?.generatedImages?.length || 0}`);
    ...
    const handleGenderChange = (val: string) => {
        setGender(val);
        const updated = { ...asset, gender: val as any };
        onUpdate(updated);
    };

    const handleAgeChange = (val: string) => {
        setAgeGroup(val);
        const updated = { ...asset, ageGroup: val as any };
        onUpdate(updated);
    };
    ...
    const handleGenerate = async () => {
        ...
        const genderLabel = t.character.genderOptions[gender as keyof typeof t.character.genderOptions] || gender;
        const ageLabel = t.character.ageOptions[ageGroup as keyof typeof t.character.ageOptions] || ageGroup;
        ...
        try {
            console.log(`[CharacterDetail] Submitting generation job for asset: ${asset.id} with count: ${generateCount}`);
            ...
            console.log(`[CharacterDetail] Added ${jobs.length} jobs`);
            ...
            console.error('[CharacterDetail] Failed to add job:', error);
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-2">
                            ...
                            >
                                {Object.entries(t.character.genderOptions).map(([key, label]) => (
                                    <SelectItem key={key} textValue={label as string}>
                                        {label as string}
                                    </SelectItem>
                                ))}
                            </Select>
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            >
                                {Object.entries(t.character.ageOptions).map(([key, label]) => (
                                    <SelectItem key={key} textValue={label as string}>
                                        {label as string}
                                    </SelectItem>
                                ))}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {asset.generatedImages?.slice().reverse().map((img) => {
                            ...
                            return (
                                <div key={img.id} className="relative group cursor-pointer" onClick={() => handleSelectImage(img)}>
                                    <Card 
                                        className={`aspect-[3/4] border-2 transition-all duration-300 ${isSelected ? 'border-indigo-600 shadow-xl scale-[1.02]' : 'border-transparent hover:border-indigo-500/50 hover:shadow-lg'}`}
                                        radius="lg"
                                        shadow="sm"
                                    >
                        ...
                        {(!asset.generatedImages || asset.generatedImages.length === 0) && (
                            <div className="col-span-full h-64 flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl bg-slate-50/50 dark:bg-slate-900/50">
                                ...
                                <span className="uppercase tracking-widest text-xs font-black opacity-50">{t.character.noGenerationsYet}</span>
                            </div>
                        )}
                    </div>
                </div>

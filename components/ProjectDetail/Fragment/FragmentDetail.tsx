  ...
  useEffect(() => {
    const loadAssets = async () => {
      ...
      if (assets) {
        ...
        setItems(
          allItems.filter(
            i => !i.itemType || i.itemType === ItemType.PROP || i.itemType === ItemType.ANIMAL
          )
        );
...
const FragmentDetail: FC<FragmentDetailProps> = ({ asset, onUpdate, projectId }) => {
  ...
  const runtimeModel =
    settings.models.find(m => m.id === modelId) || settings.models.find(m => m.type === 'video');
  ...
  useEffect(() => {
    ...
    if (ratioParams?.options && !ratioParams.options.some(o => o.value === ratio)) {
      setRatio(
        (ratioParams.defaultValue as string) || (ratioParams.options[0]?.value as string) || '16:9'
      );
    }
    ...
    if (resParams?.options && !resParams.options.some(o => o.value === resolution)) {
      setResolution(
        (resParams.defaultValue as string) || (resParams.options[0]?.value as string) || '720p'
      );
    }
  }, [modelId, modelConfig]);
  ...
  useEffect(() => {
    ...
    const checkActiveJobs = async () => {
      ...
      const fragmentJobs = jobs.filter(
        j =>
        ...
        if (job.status === JobStatus.COMPLETED || job.status === JobStatus.FAILED) {
          ...
          if (job.status === JobStatus.COMPLETED) {
            ...
            if (job.type === 'generate_image' && job.result?.path) {
              const isStart =
                job.params.assetName?.includes('_start_frame') ||
                job.params.userPrompt?.includes('start frame');
              const isEnd =
                job.params.assetName?.includes('_end_frame') ||
                job.params.userPrompt?.includes('end frame');
            ...
            try {
              ...
              const finalAsset = (await storageService.getAsset(
                asset.id,
                projectId
              )) as FragmentAsset;
  ...
  const buildFullPrompt = () => {
    ...
    if (generationType === 'text_to_video') {
      const charPrompts = selectedCharacters
        .map(id => characters.find(c => c.id === id)?.prompt)
        .filter(Boolean)
        .join(', ');
      const itemPrompts = selectedItems
        .map(id => items.find(i => i.id === id)?.prompt)
        .filter(Boolean)
        .join(', ');
      const effectPrompts = selectedEffects
        .map(id => effects.find(e => e.id === id)?.prompt)
        .filter(Boolean)
        .join(', ');
    ...
    if (generationType === 'multi_ref') {
      ...
      if (selectedRefs.length > 0) {
        const indices = selectedRefs.map(() =>
          labels.imageIndex.replace('{index}', String(imageIndex++))
        );
    ...
    try {
      ...
      const jobs = aiService.createVideoGenerationJobs(
        modelConfig,
        {
          ...
          ratio,
          startImage:
            generationType === 'first_last_frame' && modelConfig.capabilities?.supportsStartFrame
              ? startImage
              : undefined,
          endImage:
            generationType === 'first_last_frame' && modelConfig.capabilities?.supportsEndFrame
              ? endImage
              : undefined,
    ...
    try {
      ...
      const selectedResources = [
        ...selectedCharacters.map(id => ({
          ...
          _type: t.project.characters,
        })),
        ...selectedItems.map(id => ({ ...items.find(i => i.id === id)!, _type: t.project.items })),
        ...(selectedScene
          ? [scenes.find(s => s.id === selectedScene)!].map(s => ({ ...s, _type: t.project.scene }))
          : []),
      ...
      const isVidu =
        imgModelConfig.provider === 'vidu' || imgModelConfig.id.toLowerCase().includes('vidu');
        ...
        for (let i = 0; i < selectedResources.length; i++) {
          ...
          if (refImages.length < effectiveMaxRefs && res.filePath) {
            ...
            if (file && file.size > 10 * 1024 * 1024) {
              showToast(
                `${t.errors?.fileTooLarge || 'File too large'}: ${res.name} (>10MB)`,
                'error'
              );
      ...
      if (requiresImg && refImages.length === 0) {
        showToast(
          t.errors?.imageRequired || 'Image input required for this model (min 1 image)',
          'error'
        );
        setImgGenLoading(false);
        return;
  ...
  // Validation
  const isFormValid =
    !!modelId && !!prompt.trim() && (generationType !== 'first_last_frame' || !!startImage);
    ...
    <div className="flex h-full w-full gap-4 p-4">
      ...
      <Card className="w-1/3 min-w-[350px] h-full flex flex-col overflow-hidden">
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Tabs
              ...
              disabledKeys={['text_to_video', 'first_last_frame', 'multi_ref'].filter(
                type => !isTypeSupported(type)
              )}
            >
          ...
          <div className="flex flex-col gap-3 border-y py-4 border-slate-200 dark:border-slate-800">
            ...
            <Accordion
              ...
              defaultExpandedKeys={['characters']}
              motionProps={{}}
            >
              {/* Characters */}
              <AccordionItem
                key="characters"
                title={t.project.character}
                subtitle={`${selectedCharacters.length}/7`}
              >
                      ...
                      >
                        ...
                        <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                          <Button
                            ...
                            className="h-6 w-6 min-w-0 bg-black/50 text-white"
                            onClick={(e: any) => {
                              e.stopPropagation();
                              if (c.filePath && urlCache[c.filePath])
                                openPreview([{ src: urlCache[c.filePath] }]);
                            }}
                          >
                            <Eye size={12} />
                          </Button>
              ...
              <AccordionItem
                key="items"
                title={t.project.item}
                subtitle={`${selectedItems.length}/5`}
              >
                <div className="grid grid-cols-3 gap-2">
                  {items.map(i => {
                      ...
                      >
                        ...
                        <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                          <Button
                            ...
                            onClick={(e: any) => {
                              e.stopPropagation();
                              if (i.filePath && urlCache[i.filePath])
                                openPreview([{ src: urlCache[i.filePath] }]);
                            }}
                          >
                            <Eye size={12} />
                          </Button>
              ...
              <AccordionItem
                key="scenes"
                title={t.project.scene}
                subtitle={selectedScene ? '1/1' : '0/1'}
              >
                <div className="grid grid-cols-3 gap-2">
                  {scenes.map(s => {
                      ...
                      >
                        ...
                        <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                          <Button
                            ...
                            onClick={(e: any) => {
                              e.stopPropagation();
                              if (s.filePath && urlCache[s.filePath])
                                openPreview([{ src: urlCache[s.filePath] }]);
                            }}
                          >
                            <Eye size={12} />
                          </Button>
              ...
              <AccordionItem
                key="effects"
                title={t.project.fragment.effects}
                subtitle={`${selectedEffects.length}/2`}
              >
                <div className="grid grid-cols-3 gap-2">
                  {effects.map(e => {
                      ...
                      >
                        ...
                        <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                          <Button
                            ...
                            onClick={ev => {
                              ev.stopPropagation();
                              if (e.filePath && urlCache[e.filePath])
                                openPreview([{ src: urlCache[e.filePath] }]);
                            }}
                          >
                            <Eye size={12} />
                          </Button>
              ...
              <AccordionItem
                key="refs"
                title={t.project.itemTypes.reference}
                subtitle={`${selectedRefs.length}/2`}
              >
                <div className="grid grid-cols-3 gap-2">
                  {refs.map(r => {
                      ...
                      >
                        ...
                        <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                          <Button
                            ...
                            onClick={e => {
                              e.stopPropagation();
                              if (r.filePath && urlCache[r.filePath])
                                openPreview([{ src: urlCache[r.filePath] }]);
                            }}
                          >
                            <Eye size={12} />
                          </Button>
                  ...
                  {startImage ? (
                      ...
                      <>
                        ...
                        <div className="absolute top-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                          <Button
                            ...
                            variant="flat"
                            aria-label={t.common?.preview || 'Preview'}
                            className="h-6 w-6 min-w-0 bg-black/50 text-white rounded-full"
                            onPress={() => {
                              if (urlCache[startImage])
                                openPreview([{ src: urlCache[startImage] }]);
                            }}
                          >
                            <Eye size={12} />
                          </Button>
                          <Button
                            ...
                            className="h-6 w-6 min-w-0 bg-danger/80 text-white rounded-full"
                            onPress={() => {
                              setStartImage('');
                            }}
                          >
                    ...
                    <div className="flex flex-col gap-2 p-2 w-full">
                      <Button
                        ...
                        color="primary"
                        variant="flat"
                        onPress={() => openImageGenModal('start')}
                      >
        ...
        <div className="p-4 border-t bg-slate-50 dark:bg-slate-900/50">
          <Button
            className="w-full font-bold shadow-xl shadow-lime-500/30 hover:shadow-lime-500/50 transition-all hover:scale-[1.01]"
            size="lg"
      ...
      <div className="flex-1 h-full overflow-y-auto bg-slate-50 dark:bg-slate-900 rounded-xl p-4">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold">
            {t.project.fragment.results} ({asset.videos?.length || 0})
          </h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {Array.from(activeJobIds).map(jobId => (
            <Card
              key={jobId}
              className="aspect-video flex items-center justify-center bg-slate-200 dark:bg-slate-800 animate-pulse relative"
            >
              ...
              <div className="absolute bottom-0 left-0 w-full p-2 bg-black/40 text-white">
                <p className="text-[10px] font-bold truncate">
                  {jobNames[jobId] || asset.videoName || asset.name}
                </p>
              </div>
            </Card>
          ))}
                ...
                {!activeJobIds.has(video.id) && (
                  <div className="absolute bottom-0 left-0 w-full p-2 bg-gradient-to-t from-black/90 to-transparent text-white">
                    <p className="text-xs font-bold truncate">
                      {video.name || video.params?.assetName || asset.videoName || asset.name}
                    </p>

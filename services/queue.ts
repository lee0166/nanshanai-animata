...
export class JobQueue {
  ...
  private async updateAssetWithResult(job: Job, resultPaths: string[]) {
    ...
    try {
      console.log(
        `[Queue] Updating asset ${job.params.assetId} with ${resultPaths.length} images. Project: ${projectId}`
      );

      await storageService.updateAsset(job.params.assetId, projectId, async asset => {
        console.log(
          `[Queue] Current asset state: type=${asset.type}, images=${(asset as CharacterAsset).generatedImages?.length || 0}`
        );
        ...
        if (job.type === 'generate_image') {
          ...
          console.log(
            `[Queue] Saving asset ${charAsset.id}. Old count: ${oldCount}, New count: ${charAsset.generatedImages.length}`
          );
          return charAsset;
  ...
  private async processQueue() {
    ...
    // Idle interval when no jobs are active (10 seconds to reduce log noise)
    const idleInterval = 10000;

    // Get all active jobs (PROCESSING)
    const activeJobs = await storageService
      .getJobs()
      .then(jobs => jobs.filter(j => j.status === JobStatus.PROCESSING));
    ...
    try {
      ...
      const staleJobs = allJobs.filter(
        j => j.status === JobStatus.PROCESSING && !this.inFlightIds.has(j.id)
      );
      if (staleJobs.length > 0) {
        console.log(
          `[Queue] Found ${staleJobs.length} stale PROCESSING jobs (not in memory). Handling recovery...`
        );
        ...
        for (const job of staleJobs) {
          if (job.type === 'generate_image') {
              ...
              try {
                ...
                if (Array.isArray(generatedImages) && generatedImages.length > 0) {
                  const candidates = generatedImages
                    .filter(
                      img =>
                        img && typeof img.path === 'string' && typeof img.createdAt === 'number'
                    )
            ...
            if (job.params.taskId) {
              console.log(
                `[Queue] Resuming stale video job ${job.id} with taskId: ${job.params.taskId}`
              );
      ...
      if (pendingJobs.length > 0 || this.processingCount > 0) {
        console.log(
          `[Queue] Status: ${this.processingCount}/${maxJobs} active. Found ${pendingJobs.length} pending jobs.`
        );
      }

      if (pendingJobs.length === 0) {
        // No pending jobs, use longer idle interval to reduce unnecessary polling and log noise
        const nextInterval = this.processingCount > 0 ? pollInterval : idleInterval;
  ...
  private async executeJob(currentJob: Job) {
    ...
    console.log(
      `[Queue] STARTING job ${currentJob.id} (${currentJob.type}). Active: ${this.processingCount}`
    );
    ...
    try {
      ...
      if (currentJob.type === 'generate_image') {
        ...
        for (const item of results) {
          ...
          let safeAssetName = (currentJob.params.assetName || 'default').replace(
            /[^a-zA-Z0-9\u4e00-\u9fa5_\-]/g,
            '_'
          );
      ...
      try {
        ...
        console.error(
          `[Queue] Failed to save job ${currentJob.id} status in finally block:`,
          saveError
        );
      }

        ...
        <div className="px-6 py-4 flex items-center gap-4 border-b border-slate-200 dark:border-slate-800 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500">
              {t.project.resourceManager.filter}
            </span>
          </div>
          ...
          >
            <SelectItem key="all" value="all">
              {t.project.resourceManager.allTypes}
            </SelectItem>
            <SelectItem key="image" value="image">
              {t.project.resourceManager.images}
            </SelectItem>
            <SelectItem key="video" value="video">
              {t.project.resourceManager.videos}
            </SelectItem>
          </Select>
          ...
          >
            <SelectItem key="all" value="all">
              {t.project.resourceManager.allSources}
            </SelectItem>
            <SelectItem key="generated" value="generated">
              {t.project.resourceManager.generated}
            </SelectItem>
            <SelectItem key="imported" value="imported">
              {t.project.resourceManager.imported}
            </SelectItem>
          </Select>
                                ...
                                [
                                  {
                                    type: 'video',
                                    sources: [
                                      { src: url, type: getMimeType(selectedResource.path) },
                                    ],
                                  },
                                ],
                                0
                          ...
                          <Chip variant="flat" color="primary" className="font-bold">
                            {selectedResource.sourceAssetType === AssetType.VIDEO_SEGMENT
                              ...
                              : selectedResource.sourceAssetType === AssetType.CHARACTER
                                ...
                                : selectedResource.sourceAssetType === AssetType.SCENE
                                  ? t.project.scenes
                                  : selectedResource.sourceAssetType === AssetType.ITEM
                                    ? t.project.items
                                    : t.project.resources}
                          </Chip>
                          <Chip
                        ...
                        <div className="space-y-6">
                          ...
                          <div>
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-3">
                              {t.project.resourceManager.fileInfo}
                            </label>
                            <div className="space-y-3">
                              <div className="flex items-center justify-between">
                                <span className="text-xs text-slate-500">
                                  {t.project.resourceManager.dimensions}
                                </span>
                                <span className="text-xs font-mono font-medium">
                                  {selectedResource.metadata?.width &&
                                  selectedResource.metadata?.height
                              ...
                              <div className="flex items-center justify-between">
                                <span className="text-xs text-slate-500">
                                  {t.project.resourceManager.size}
                                </span>
                                <span className="text-xs font-mono font-medium">
                                  {formatSize(selectedResource.metadata?.size)}
                                </span>
                              </div>
                              <div className="flex items-center justify-between">
                                <span className="text-xs text-slate-500">
                                  {t.project.resourceManager.fileType}
                                </span>
                                <span className="text-xs font-mono font-medium uppercase">
                                  {selectedResource.metadata?.fileType ||
                              ...
                              {selectedResource.type === 'video' && (
                                <>
                                  <div className="flex items-center justify-between">
                                    <span className="text-xs text-slate-500">
                                      {t.project.resourceManager.duration}
                                    </span>
                                    <span className="text-xs font-mono font-medium">
                          ...
                          {selectedResource.source === 'generated' && (
                            <div>
                              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-3">
                                {t.project.resourceManager.generationInfo}
                              </label>
                              <div className="space-y-4">
                                {selectedResource.prompt && (
                                  <div className="space-y-1">
                                    <span className="text-[10px] text-slate-400 font-bold uppercase">
                                      {t.project.resourceManager.prompt}
                                    </span>
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                  <div className="flex flex-col gap-1">
                                    <span className="text-[10px] text-slate-400 font-bold uppercase">
                                      {t.project.resourceManager.model}
                                    </span>
                                    <span className="text-xs font-medium truncate bg-slate-50 dark:bg-white/5 px-2 py-1 rounded border border-slate-100 dark:border-white/10">
                                      {selectedResource.metadata?.modelId ||
                                        t.project.resourceManager.unknown}
                                    </span>
                                  </div>
                                  {selectedResource.generationParams && (
                                    <div className="flex flex-col gap-1">
                                      <span className="text-[10px] text-slate-400 font-bold uppercase">
                                        {t.project.resourceManager.params}
                                      </span>

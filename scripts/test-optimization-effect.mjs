...
async function testSchemaValidation() {
  ...
  try {
    // 动态导入（因为ESM）
    const { ScriptMetadataSchema, ScriptCharacterSchema } = await import(
...
async function testVectorMemory() {
  ...
  try {
    const { vectorMemory } = await import(
    ...
    await vectorMemory.addDocuments(testDocs, embeddings.map(e => e.embedding));
    ...
    queryResults.forEach((result, index) => {
      console.log(`  ${index + 1}. ${result.text.substring(0, 30)}... (相关度: ${(1 - result.distance).toFixed(2)})`);
    });

...
export default defineConfig(({ mode }) => {
    ...
    middlewares.use('/api/universal-proxy', async (req: any, res: any, next: any) => {
      ...
      try {
        ...
        const stylePrompts: Record<string, string> = {
          ...
          shinkai:
            'Makoto Shinkai style, beautiful sky, lens flare, detailed background, emotional',
        ...
        console.log(
          `[Universal Proxy] ${req.method} -> ${targetUrl}${modelInfo}${promptPreview}${styleInfo}${sizeValue}`
        );
        ...
        for (const [key, value] of Object.entries(
          ...
          if (
            !['host', 'origin', 'content-length', 'connection', 'x-target-url'].includes(
              key.toLowerCase()
            )
          ) {
            if (Array.isArray(value)) {
        ...
        console.log(
          `[Universal Proxy] Response from ${targetUrl}: ${response.status} ${response.statusText}`
        );
  ...
  return {
    server: {
      ...
      // 优化开发服务器性能
      warmup: {
        clientFiles: ['./index.tsx', './App.tsx', './components/Layout.tsx'],
      },
    },
    plugins: [
      react(),

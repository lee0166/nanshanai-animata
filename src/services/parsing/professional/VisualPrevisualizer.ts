 ...
 * 色彩情绪到描述的映射
 */
const COLOR_MOOD_MAP: Record<string, {
  description: string;
  lighting: string[];
  keywords: string[];
}> = {
 ...
 * 时代关键词映射
 */
const ERA_KEYWORDS: Record<string, {
...
export class VisualPrevisualizer {
  ...
  analyze(
    ...
    // 2. 生成时代视觉参考
    const eraVisualGuide = this.generateEraVisualGuide(
      visualStyle,
      eraContext
    );

    // 3. 生成整体视觉描述
    const overallVisualDescription = this.generateOverallVisualDescription(
      visualStyle,
      eraContext
    );

    // 4. 生成场景视觉建议
    const sceneVisualSuggestions = this.generateSceneVisualSuggestions(
      visualStyle
    );
   ...
   * 生成场景视觉建议
   */
  private generateSceneVisualSuggestions(
    visualStyle: VisualStyle
  ): SceneVisualSuggestion[] {
    ...
    suggestions.push({
      sceneType: '室内场景',
      visualStyle: `${visualStyle.artDirection}风格，` +
        `利用${moodData.lighting[0] || '自然光'}营造氛围`,
      colorSuggestion: `以${visualStyle.colorPalette[0] || '主色调'}为主，` +
    ...
    suggestions.push({
      sceneType: '室外场景',
      visualStyle: `${visualStyle.cinematography}，` +
        `展现${moodData.keywords[0] || '环境'}氛围`,

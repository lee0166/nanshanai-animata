    ...
    try {
      switch (stage) {
        ...
        case 'characters': {
          ...
          try {
            // Use batch extraction with context injection for all characters
            // This ensures characters are consistent with story context and visual style
            const characters = await this.extractAllCharactersWithContext(content, state.metadata.characterNames || []);
            state.characters = characters;
            console.log(`[ScriptParser] Batch extracted ${characters.length} characters with context in 1 API call`);
          } catch (e) {
            console.error('[ScriptParser] Batch character extraction with context failed, falling back to individual extraction:', e);
            // Fallback to individual extraction with context
            ...
            for (let i = 0; i < characterNames.length; i++) {
              ...
              try {
                // Use context-enhanced extraction if global context is available
                const character = this.globalContext && this.contextInjector
                  ? await this.extractCharacterWithContext(content, name)
                  : await this.extractCharacter(content, name);
                characters.push(character);
              } catch (e) {
                console.error(`Failed to extract character ${name}:`, e);
                characters.push({
        ...
        case 'scenes': {
          ...
          try {
            // Use batch extraction with context injection for all scenes
            // This ensures scenes are consistent with visual style and era context
            const scenes = await this.extractAllScenesWithContext(content, state.metadata.sceneNames || []);
            state.scenes = scenes;
            console.log(`[ScriptParser] Batch extracted ${scenes.length} scenes with context in 1 API call`);
          } catch (e) {
            console.error('[ScriptParser] Batch scene extraction with context failed, falling back to individual extraction:', e);
            // Fallback to individual extraction with context
            ...
            for (let i = 0; i < sceneNames.length; i++) {
              ...
              try {
                // Use context-enhanced extraction if global context is available
                const scene = this.globalContext && this.contextInjector
                  ? await this.extractSceneWithContext(content, name)
                  : await this.extractScene(content, name);
                scenes.push(scene);
              } catch (e) {
                console.error(`Failed to extract scene ${name}:`, e);
                scenes.push({
        ...
        case 'shots': {
          ...
          try {
            // Use batch generation with context injection for all shots
            // This ensures shots follow the visual guidance and emotional context
            const allShots = await this.generateAllShotsWithContext(content, state.scenes);
            state.shots = allShots;
            console.log(`[ScriptParser] Batch generated ${allShots.length} shots with context in 1 API call`);
          } catch (e) {
            console.error('[ScriptParser] Batch shots generation with context failed, falling back to individual generation:', e);
            // Fallback to individual generation with context
            const allShots: Shot[] = [];
            ...
            for (let i = 0; i < state.scenes.length; i++) {
              ...
              try {
                // Use context-enhanced generation if global context is available
                const shots = this.globalContext && this.contextInjector
                  ? await this.generateShotsWithContext(content, scene, i, state.scenes.length)
                  : await this.generateShots(
                      content,
                      scene.name,
                      scene.description,
                      scene.characters,
          ...
          if (this.parserConfig.useDramaRules && this.qualityAnalyzer) {
            console.log('[ScriptParser] ========== Generating Quality Report ==========');
            console.log('[ScriptParser] Input data:', {
              hasMetadata: !!state.metadata,
              charactersCount: this.currentCharacters.length,
              scenesCount: this.currentScenes.length,
            ...
            console.log('[ScriptParser] Violations:', finalReport.violations.length);
            console.log('[ScriptParser] Suggestions:', finalReport.suggestions.length);
            console.log('[ScriptParser] Report type:', typeof finalReport);
            console.log('[ScriptParser] Has dimensionScores:', !!finalReport.dimensionScores);
            console.log('[ScriptParser] Has statistics:', !!finalReport.statistics);
          }

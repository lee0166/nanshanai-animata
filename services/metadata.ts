  ...
  try {
    ...
    while (offset < bytes.length - 8) {
      ...
      if (
        ...
        console.log(
          `[Metadata] Found mdhd atom at ${offset}. Version: ${version}, Timescale: ${timescale}`
        );
      }
      ...
      if (
        ...
        if (entryCount > 0) {
          ...
          console.log(
            `[Metadata] Found stts atom at ${offset}. First sample delta: ${sampleDelta}`
          );
        }
      }
      ...
      if (fileSize > 256 * 1024) {
        ...
        for (let i = 0; i < footerBytes.length - 8; i++) {
          ...
          if (
            !timescale &&
            footerBytes[i] === 0x6d &&
            footerBytes[i + 1] === 0x64 &&
            footerBytes[i + 2] === 0x68 &&
            footerBytes[i + 3] === 0x64
          ) {
            const version = footerView.getUint8(i + 4);
            timescale = footerView.getUint32(i + (version === 0 ? 16 : 24));
            console.log(
              `[Metadata] Found mdhd atom in footer at ${i}. Version: ${version}, Timescale: ${timescale}`
            );
          }
          // Check for 'stts'
          if (
            !sampleDelta &&
            footerBytes[i] === 0x73 &&
            footerBytes[i + 1] === 0x74 &&
            footerBytes[i + 2] === 0x74 &&
            footerBytes[i + 3] === 0x73
          ) {
            const entryCount = footerView.getUint32(i + 8);
            if (entryCount > 0) {
              sampleDelta = footerView.getUint32(i + 16);
              console.log(
                `[Metadata] Found stts atom in footer at ${i}. First sample delta: ${sampleDelta}`
              );
            }
          }
          if (timescale && sampleDelta) break;

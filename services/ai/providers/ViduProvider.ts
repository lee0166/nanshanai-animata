...
export class ViduProvider extends BaseProvider {
  ...
  async generateImage(
    ...
    try {
      ...
      if (referenceImages && referenceImages.length > 0) {
        ...
        return {
          success: false,
          error: `${config.modelId} requires at least one reference image.`,
        };
      }

      console.log(
        `[ViduProvider] Creating video task at ${baseUrl}${endpoint} with model ${config.modelId}`
      );
  ...
  async generateVideo(
    ...
    try {
      ...
      const offPeak =
        extraParams?.offPeak !== undefined ? extraParams.offPeak : extraParams?.off_peak;
      ...
      const isReferenceToVideo =
        config.capabilities?.supportedGenerationTypes?.includes('multi_ref') && hasRefs;
      ...
      if (isReferenceToVideo) {
        ...
        if (imageUrls.length === 0) {
          return {
            success: false,
            error: 'Failed to load any reference images for multi-reference video generation.',
          };
        }
    ...
    for (let i = 0; i < maxAttempts; i++) {
      try {
        ...
        if (data.state === 'failed') {
          return {
            success: false,
            error: `Vidu Task Failed: ${data.err_code || 'Unknown error'}`,
            metadata: data,
          };
        }

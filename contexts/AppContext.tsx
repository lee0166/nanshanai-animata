...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [hasPreviousWorkspace, setHasPreviousWorkspace] = useState(false);
  ...
  const reloadSettings = async () => {
    ...
    try {
      ...
      if (connected) {
        ...
        let pollingInterval = Number.isFinite(parsedPolling)
          ? parsedPolling
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        // Check if there's a previous workspace selection
        const hasPrevious = await storageService.hasPreviousWorkspace();
        if (mounted) {
          setHasPreviousWorkspace(hasPrevious);
        }
        console.log('[INIT] Has previous workspace:', hasPrevious);
        ...
        if (mounted) {
          ...
          console.log(
            `[INIT] Initialization complete. FS Responsive: ${storageService.isResponsive()}, Connected: ${connected}, HasPrevious: ${hasPrevious}, Elapsed: ${Date.now() - start}ms`
          );
          setIsInitializing(false);
          setLoaded(true);
        }
        ...
        if (mounted) {
          console.log(
            `[INIT] Initialization failed with error. FS Responsive: ${storageService.isResponsive()}`
          );
          setIsFsResponsive(storageService.isResponsive());
          setIsInitializing(false);
          setLoaded(true);
  ...
  const reconnectWorkspace = async () => {
    try {
      console.log('[AppContext] Attempting to reconnect to previous workspace...');
      const granted = await storageService.requestPermissionForExisting();
      if (granted) {
        console.log('[AppContext] Permission granted, reloading...');
        window.location.reload();
        return true;
      } else {
        console.warn('[AppContext] Permission denied');
        return false;
      }
    } catch (e) {
      console.error('[AppContext] Reconnect failed:', e);
      return false;
    }
  };
        ...
        <div className="fixed inset-0 bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center z-50 p-6 text-center">
          <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-xl max-w-md border border-red-100 dark:border-red-900/30">
            ...
            <p className="text-slate-600 dark:text-slate-400 mb-8">
              {t.settings.browserNotSupportedDesc}
            </p>
          </div>
        </div>

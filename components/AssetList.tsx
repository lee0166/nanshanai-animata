...
import { Asset, AssetType, CharacterAsset, ItemAsset } from '../types';
...
const AssetList: React.FC<AssetListProps> = ({
  ...
  const itemTypeFilter =
    externalItemTypeFilter !== undefined ? externalItemTypeFilter : internalItemTypeFilter;
  const _setItemTypeFilter = (val: string) => {
  ...
  useEffect(() => {
    if (highlightedAssetId && assetRefs.current[highlightedAssetId]) {
      assetRefs.current[highlightedAssetId]?.scrollIntoView({
        behavior: 'smooth',
        block: 'center',
      });
    }
    ...
    if (type === AssetType.RESOURCES && path) {
      ...
      for (const asset of allAssets) {
        ...
        if (asset.filePath === path) {
          associated.push(
            `${t.project[asset.type as keyof typeof t.project] || asset.type}: ${asset.name}`
          );
          continue;
        }
  ...
  const displayAssets = useMemo(() => {
    ...
    if (
      ...
      console.log(
        ...
        result.map(a => ({ id: a.id, name: a.name, scriptId: a.scriptId }))
      );
      result = result.filter(asset => {
    ...
    <>
        ...
        {isResources && (
          <div className="flex flex-col sm:flex-row items-center justify-start gap-6 bg-white/50 dark:bg-slate-900/50 backdrop-blur-md p-4 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-x-auto">
            <Tabs
              ...
              classNames={{
                tabList:
                  'bg-slate-100 dark:bg-slate-950 p-1 rounded-2xl border border-slate-200 dark:border-slate-800',
      ...
      <Modal isOpen={isEditOpen} onOpenChange={onEditOpenChange} placement="center" backdrop="blur">
        <ModalContent>
          {onClose => (
            <>
              <ModalHeader className="flex flex-col gap-1">{t.project.edit}</ModalHeader>
              <ModalBody>
                <Input
